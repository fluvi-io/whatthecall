...
function utf8ToHex(str) {
    return '0x' + Array.from(str).map(c => c.charCodeAt(0) < 128 ? c.charCodeAt(0).toString(16) :
        encodeURIComponent(c).replace(/\%/g, '').toLowerCase()).join('');
}
const main = () => __awaiter(void 0, void 0, void 0, function* () {
    console.log(yield explain("klay-token", { to: "0x754288077d0ff82af7a5317c7cb8c444d421d103", data: "0xa9059cbb000000000000000000000000f1adb4a5b8ed1621eccf97bfac10491c97f79c9400000000000000000000000000000000000000000000000000000000dea7ecd6" }));
});
...
const signature = memoizeAsync((a) => __awaiter(void 0, void 0, void 0, function* () { const r = yield fourByte(a); return r[r.length - 1]; }));
...
const explain = (p, tx) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (sig in functionsCompiled) {
        return yield functionsCompiled[sig](platform(p), tx, params);
    }
    else {
        return { title: `${sig}` };
    }
...
const debug = (a) => { console.log(a); return a; };
const functionsCompiled = Object.fromEntries(Object.entries(functions).map(([k, v]) => [toSignature(k), (p, tx, args) => v(p, tx, debug(Object.fromEntries(zip(paramNames(k), args))))]));
main().catch((error) => {
    console.error(error);
    process.exitCode = 1;
});
